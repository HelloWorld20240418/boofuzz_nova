...
def fuzzing_main():
    ...
    ConnectRequest(session)
    ReadyToUse(session)
    RC_SEND_READ_WRITE(session)
    Acknowledge_QP(session)
    DisconnectRequest(session)
    DisconnectReply(session)
    session.fuzz()

def ConnectRequest(session):
    s_initialize(name="ConnectRequest_InfiniBand")
    with s_block("Base_Transport_Header"):
        s_bytes(value=bytes([0x64]), size=1, max_len=1, name='Opcode', fuzzable=False)
    ...
    with s_block("MAD_Header"):
        ...
        s_bytes(value=bytes([0x30,0x00,0x00,0x00]), size=4, max_len=4, name='Attribute Modifier', fuzzable=True)

    with s_block("CM ConnectRequest"):
        s_bytes(value=bytes([0x09,0x56,0xef,0x36]), size=4, max_len=4, name='Local Communication ID', fuzzable=True)
        ...
        s_bytes(value=bytes([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,0x0a,0x00,0x00,0x02]), size=16, max_len=16, name='Primary Local Port GID', fuzzable=True)
    ...
    session.connect(s_get('ConnectRequest_InfiniBand'))

def ReadyToUse(session):
    s_initialize(name="ReadyToUse_InfiniBand")
    with s_block("Base_Transport_Header"):
        s_bytes(value=bytes([0x64]), size=1, max_len=1, name='Opcode', fuzzable=False)
        s_bytes(value=bytes([0x40]), size=1, max_len=1, name='MigReq', fuzzable=False)
        s_bytes(value=bytes([0xff,0xff]), size=2, max_len=2, name='Partition Key', fuzzable=False)
    ...
    session.connect(s_get('ReadyToUse_InfiniBand'))
...
def Acknowledge_QP(session):
    s_initialize(name="Acknowledge_QP_InfiniBand")
    with s_block("Base_Transport_Header"):
        s_bytes(value=bytes([0x11]), size=1, max_len=1, name='Opcode', fuzzable=False)
        s_bytes(value=bytes([0x40]), size=1, max_len=1, name='MigReq', fuzzable=False)
        s_bytes(value=bytes([0xff,0xff]), size=2, max_len=2, name='Partition Key', fuzzable=False)
        s_bytes(value=bytes([0x00]), size=1, max_len=1, name='Reserved1', fuzzable=True)
        s_bytes(value=bytes([0x00,0x11,0xff]), size=3, max_len=3, name='Destination Queue Pair', fuzzable=False)
        s_bytes(value=bytes([0x80]), size=1, max_len=1, name='Acknowledge Request', fuzzable=False)
        s_bytes(value=bytes([0x94,0xdc,0xb0]), size=3, max_len=3, name='Packet Sequence Number', fuzzable=False)
    with s_block("AETH - ACK Extended Transport Header"):
        s_bytes(value=bytes([0x00]), size=1, max_len=1, name='OpCode: Ack', fuzzable=False)
        s_bytes(value=bytes([0x00,0x00,0x01]), size=3, max_len=3, name='Message Sequence Number', fuzzable=False)
        s_bytes(value=bytes([0x9d,0x96,0x36,0xd4]), size=4, max_len=4, name='Invariant CRC', fuzzable=False)
    session.connect(s_get('Acknowledge_QP_InfiniBand'))

def DisconnectRequest(session):
    s_initialize(name="DisconnectRequest_InfiniBand")
    with s_block("Base_Transport_Header"):
        s_bytes(value=bytes([0x64]), size=1, max_len=1, name='Opcode', fuzzable=False)
        s_bytes(value=bytes([0x40]), size=1, max_len=1, name='MigReq', fuzzable=False)
        s_bytes(value=bytes([0xff,0xff]), size=2, max_len=2, name='Partition Key', fuzzable=False)
    ...
    with s_block("CM ConnectRequest"):
        ...
        s_bytes(value=bytes([0x00]), size=1, max_len=1, name='Reserved4', fuzzable=True)
        s_bytes(value=bytes(
    ...
    session.connect(s_get('DisconnectRequest_InfiniBand'))

def DisconnectReply(session):
    s_initialize(name="DisconnectReply_InfiniBand")
    with s_block("Base_Transport_Header"):
        s_bytes(value=bytes([0x64]), size=1, max_len=1, name='Opcode', fuzzable=False)
        s_bytes(value=bytes([0x40]), size=1, max_len=1, name='MigReq', fuzzable=False)
        s_bytes(value=bytes([0xff,0xff]), size=2, max_len=2, name='Partition Key', fuzzable=False)
        ...
        s_bytes(value=bytes([0x00,0x00,0x01]), size=3, max_len=3, name='Destination Queue Pair', fuzzable=False)
        s_bytes(value=bytes([0x00]), size=1, max_len=1, name='Acknowledge Request', fuzzable=False)
        s_bytes(value=bytes([0x00,0x39,0x71]), size=3, max_len=3, name='Packet Sequence Number', fuzzable=False)
    ...
    with s_block("MAD_Header"):
        ...
        s_bytes(value=bytes([0x00,0x00]), size=2, max_len=2, name='Status', fuzzable=False)
        s_bytes(value=bytes([0x00,0x00]), size=2, max_len=2, name='Class Specific', fuzzable=False)
        s_bytes(value=bytes([0x00,0x00,0x00,0x08,0x1b,0x5c,0x48,0x0b]), name="Transaction ID", fuzzable=False)
        s_bytes(value=bytes([0x00,0x16]), size=2, max_len=2, name='Attribute ID', fuzzable=False)
        s_bytes(value=bytes([0x00,0x00]), size=2, max_len=2, name='Reserved3', fuzzable=True)
        s_bytes(value=bytes([0x00,0x00,0x00,0x00]), size=4, max_len=4, name='Attribute Modifier', fuzzable=False)

    with s_block("CM DisconnectReply"):
        s_bytes(value=bytes([0x0a9,0x56,0xef,0x36]), size=4, max_len=4, name='Local Communication ID', fuzzable=False)
        s_bytes(value=bytes([0x0b,0x48,0x5c,0x1b]), size=4, max_len=4, name='Remote Communication ID', fuzzable=True)
        s_string("000000000", size=224, max_len=224, name='PrivateData', fuzzable=False)
        s_bytes(value=bytes([0x4e,0x8c,0xca,0xee]), size=4, max_len=4, name='Invariant CRC', fuzzable=False)
    session.connect(s_get('DisconnectReply_InfiniBand'))
