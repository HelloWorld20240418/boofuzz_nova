...
def fuzzing_define_proto(session):
    ...
    with s_block("ams"):
        ...
        s_bytes(value=bytes([0xc0,0xa8,0x01,0x63,0x01,0x01]), size=6, max_len=6, name='ams.sendernetid', fuzzable=True)
        ...
        s_bytes(value=bytes([0x02,0x00]), size=2, max_len=2, name='stateflags', fuzzable=True)
        s_bytes(value=bytes([0x00,0x00,0x00,0x00]), size=4, max_len=4, name='cbdata', fuzzable=False)
        s_bytes(value=bytes([0x00,0x00,0x00,0x00]), size=4, max_len=4, name='errorcode', fuzzable=False)
        s_bytes(value=bytes([0x0d,0x00,0x00,0x00]), size=4, max_len=4, name='invokeid', fuzzable=True)
        session.connect(s_get('ams'))
